...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // Vercel percent-encodes this one, so a city with a space in its name
    // arrives escaped. A decode failure falls back to the raw value rather
    // than dropping the visit.
